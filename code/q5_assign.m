...
%          y(j,m) ∈ {0,1}
%  说明：
%   1) 同机多弹（同 θ/v）的遮蔽窗口时段唯一、无法接力（精确预扫描
%      结论），故 y(j,m) 取 0/1，不按弹数理想化累加；
...
%         由 q5_prescan.py 精确网格扫描生成（data/q5_prescan_pot.csv）
%   k   - 候选套数（默认 3：最优 + 前 k-1 个次优）
% 输出：
%   cand - 1 x k 结构体数组；cand(t).missile{m} = n x 2 服务表
